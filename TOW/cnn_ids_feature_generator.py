...
class CNNIDSFeatureGenerator(abstract_feature_generator.AbstractFeatureGenerator):
    ...
    def __tow_ids_dataset_generate_features(self, paths_dictionary: typing.Dict):
        ...
        #ADDITION
        chunk_size = 150467
        for i in range(0, len(aggregated_X), chunk_size):
          x_array = np.array(aggregated_X[i:i+chunk_size], dtype='uint8')
          np.savez(f"{paths_dictionary['output_path']}/X_{self._data_suffix}_{self._output_path_suffix}_{i}", x_array)
    ...
    def __aggregate_based_on_window_size(self, x_data, y_data):
        ...
        # Make final arrays
        # chunk_size = 150467  # Adjust as needed
        # x_array_list = [np.stack(X[i:i+chunk_size]).astype(np.uint8) for i in range(0, len(X), chunk_size)]
